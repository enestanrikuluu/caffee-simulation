...
from cafe_sim.calibration_extractor import extract_period_statistics, generate_threshold_diagnostics
from cafe_sim.period_key import (
    ALL_PERIOD_KEYS, MONDAY_MORNING, MONDAY_AFTERNOON, MONDAY_EVENING,
    TUESDAY_MORNING, TUESDAY_AFTERNOON, TUESDAY_EVENING,
    period_key_to_display_name
)
...
def main():
    ...
    period_statistics = extract_period_statistics(
    ...
    for period_key in ALL_PERIOD_KEYS:
        ...
        if lambda_val is not None:
            print(f"  Poisson lambda: {lambda_val:.3f} per min")
        else:
            print(f"  Poisson lambda: N/A")
...
def _suggest_barista_count(period_key):
    suggestions = {
        MONDAY_MORNING: 3,
        MONDAY_AFTERNOON: 4,
        MONDAY_EVENING: 2,
