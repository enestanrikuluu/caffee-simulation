...
def fit_all_distributions(data, variable_name="unknown", num_bins=10, min_expected=5.0):
    ...
    with warnings.catch_warnings():
        ...
        if sample_stats["min"] >= 0:
            fit_results.append(_fit_exponential(data, num_bins, min_expected))
            fit_results.append(_fit_gamma(data, num_bins, min_expected))
            fit_results.append(_fit_weibull(data, num_bins, min_expected))

            if sample_stats["min"] > 0:
                fit_results.append(_fit_lognormal(data, num_bins, min_expected))

        fit_results.append(_fit_normal(data, num_bins, min_expected))
        fit_results.append(_fit_uniform(data, num_bins, min_expected))

    fit_results.sort(key=lambda x: x['aic'])
    for rank, result in enumerate(fit_results, start=1):
        result['rank'] = rank

    recommended = _select_recommended_distribution(fit_results, data)

    return create_fitting_report(
        variable_name=variable_name,
        ...
        recommendation_reason=_generate_recommendation_reason(recommended, fit_results),
    )


def _fit_exponential(data, num_bins, min_expected):
    ...
    chi2_stat, chi2_p = _chi_square_test(
        data, lambda x: stats.expon.cdf(x, scale=1/rate), num_bins, min_expected
    )
    ...
    return create_fit_result(
        distribution=DIST_EXPONENTIAL,
        parameters=params,
...
def _fit_gamma(data, num_bins, min_expected):
    try:
    ...
    chi2_stat, chi2_p = _chi_square_test(
        data, lambda x: stats.gamma.cdf(x, shape, loc=0, scale=scale), num_bins, min_expected
    )
    ...
    ad_stat, ad_crit = _anderson_darling_generic(
        data, lambda x: stats.gamma.cdf(x, shape, loc=0, scale=scale)
    )
...
def _fit_weibull(data, num_bins, min_expected):
    try:
    ...
    chi2_stat, chi2_p = _chi_square_test(
        data, lambda x: stats.weibull_min.cdf(x, shape, loc=0, scale=scale), num_bins, min_expected
    )
...
def _fit_lognormal(data, num_bins, min_expected):
    try:
    ...
    chi2_stat, chi2_p = _chi_square_test(
        data, lambda x: stats.lognorm.cdf(x, shape, loc=0, scale=scale), num_bins, min_expected
    )
...
def _fit_normal(data, num_bins, min_expected):
    ...
    chi2_stat, chi2_p = _chi_square_test(
        data, lambda x: stats.norm.cdf(x, loc=mu, scale=sigma), num_bins, min_expected
    )
...
def _fit_uniform(data, num_bins, min_expected):
    ...
    chi2_stat, chi2_p = _chi_square_test(
        data, lambda x: stats.uniform.cdf(x, loc=a, scale=b-a), num_bins, min_expected
    )
...
def _chi_square_test(data, cdf_func, num_bins, min_expected):
    n = len(data)
    actual_num_bins = min(num_bins, int(np.sqrt(n)))

    bin_edges = np.linspace(np.min(data), np.max(data), actual_num_bins + 1)
...
def _anderson_darling_generic(data, cdf_func):
...
def _select_recommended_distribution(fit_results, data):
    ks_alpha = 0.05
    acceptable_fits = [
...
def _generate_recommendation_reason(recommended, all_results):
...
def generate_text_report(reports):
    ...
    for var_name, report in reports.items():
        ...
        text += f"\nRECOMMENDED: {report['recommended_distribution'].upper()}\n"
        text += f"Reason: {report['recommendation_reason']}\n\n"

        text += f"{'Rank':<6} {'Distribution':<15} {'AIC':<10} {'BIC':<10} "
        text += f"{'K-S p-val':<12} {'Chi2 p-val':<12} {'A-D stat':<10}\n"
        text += "-" * 100 + "\n"
        ...
        for result in report['fit_results']:
            ...
            text += f"{result['rank']:<6} {result['distribution']:<15} "
    ...
    text += "K-S p-value: >0.05 indicates good fit (fail to reject H0)\n"
    text += "Chi2 p-value: >0.05 indicates good fit (fail to reject H0)\n"
