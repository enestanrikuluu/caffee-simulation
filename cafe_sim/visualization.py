...
class VisualizationPipeline:
    ...
    def plot_calibration_service_histograms(
        self, cleaned_data: Dict[PeriodKey, pd.DataFrame], threshold: float = 1.0
    ):
        fig, axes = plt.subplots(2, 3, figsize=(15, 10))
        fig.suptitle(f'Service Time Distributions by Period (Threshold = {threshold} min)', 
                     fontsize=14, fontweight='bold')
        
        for idx, period_key in enumerate(PeriodKey):
            ax = axes[idx // 3, idx % 3]
            df = cleaned_data[period_key]
            
            if 'service_time_min' not in df.columns or len(df) == 0:
                ax.text(0.5, 0.5, 'No data', ha='center', va='center')
                ax.set_title(period_key.to_display_name())
                continue
            ...
            ax.axvline(threshold, color='red', linestyle='--', linewidth=2, label=f'Threshold ({threshold} min)')
            
            ax.set_xlabel('Service Time (min)')
            ax.set_ylabel('Frequency')
            ax.set_title(f'{period_key.to_display_name()}\nP(Drink)={len(drink_times)/len(service_times):.2f}')
            ax.legend()
            ax.grid(True, alpha=0.3)
        
        plt.tight_layout()
        output_path = self._output_dir / 'calibration_service_histograms.png'
        plt.savefig(output_path, bbox_inches='tight')
        plt.close()
        return output_path

    def plot_calibration_interarrival_ecdfs(
        ...
        plt.tight_layout()
        output_path = self._output_dir / 'calibration_interarrival_ecdfs.png'
        plt.savefig(output_path, bbox_inches='tight')
        plt.close()
        return output_path

    def plot_validation_kpi_comparison(
        self, 
        validation_results: Dict[str, AggregatedKPIs],
        ...
        for idx, (kpi_name, kpi_label) in enumerate(kpi_metrics):
            ...
            for period_key in periods:
                ...
                obs_val = observed_stats.get(period_key.value, {}).get(kpi_name, np.nan)
            ...
            ax.set_xticks(x_pos)
            ax.set_xticklabels([p.value.replace('_', '\n') for p in periods], 
                              rotation=45, ha='right', fontsize=8)
            ax.legend()
            ax.grid(True, alpha=0.3, axis='y')
        
        plt.tight_layout()
        output_path = self._output_dir / 'validation_kpi_comparison.png'
        plt.savefig(output_path, bbox_inches='tight')
        plt.close()
        return output_path

    def plot_staffing_sweep(
        self, staffing_results: Dict[int, AggregatedKPIs], period_name: str
    ):
        ...
        plt.tight_layout()
        output_path = self._output_dir / f'staffing_sweep_{period_name.replace(" ", "_").lower()}.png'
        plt.savefig(output_path, bbox_inches='tight')
        plt.close()
        return output_path

    def plot_service_mix_sensitivity(
        self, 
        service_mix_results: Dict[float, AggregatedKPIs],
        baseline_p_drink: float,
        period_name: str
    ):
        ...
        plt.tight_layout()
        output_path = self._output_dir / f'service_mix_{period_name.replace(" ", "_").lower()}.png'
        plt.savefig(output_path, bbox_inches='tight')
        plt.close()
        return output_path
    ...
    def plot_warmup_comparison(
        self, 
        warmup_results: Dict[int, AggregatedKPIs],
        period_name: str
    ):
        fig, axes = plt.subplots(1, 2, figsize=(14, 6))
        ...
        plt.tight_layout()
        output_path = self._output_dir / f'warmup_analysis_{period_name.replace(" ", "_").lower()}.png'
        plt.savefig(output_path, bbox_inches='tight')
        plt.close()
        return output_path

    def plot_threshold_diagnostics(
        self, diagnostics: Dict[PeriodKey, Dict], baseline_threshold: float
    ):
        fig, axes = plt.subplots(2, 3, figsize=(15, 10))
        ...
        plt.tight_layout()
        output_path = self._output_dir / 'threshold_diagnostics.png'
        plt.savefig(output_path, bbox_inches='tight')
        plt.close()
        return output_path

    def plot_wait_time_ecdf_comparison(
        self,
        validation_results: Dict[str, AggregatedKPIs],
        observed_data: Dict[PeriodKey, pd.DataFrame]
    ):
        fig, axes = plt.subplots(2, 3, figsize=(15, 10))
        ...
        plt.tight_layout()
        output_path = self._output_dir / 'validation_wait_ecdf.png'
        plt.savefig(output_path, bbox_inches='tight')
        plt.close()
        return output_path
